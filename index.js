...
const vcard = 'BEGIN:VCARD\n' // ANAK ANJING MAU NGAPAIN?
            ...
            + 'FN:Poks🗿\n' // MENDING LU TOBAT SU!
            + 'ORG:Creator POKS-BOT;\n' // KASIH CREDITS GUA SU!!!
            + 'TEL;type=CELL;type=VOICE;waid=6285259622026:+62 852-5962-2026\n' // JANGAN KEK BABI SU
...
client.on('qr', qr => {
   ...
   console.log(`[ ${time} ] QR code is ready, follow Ig @poukes_`)
})
...
client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Sayonara🥳 @${num.split('@')[0]} titip gorengan buat yang left/terkick!.`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
	...
	client.on('message-new', async (mek) => {
		try {
			...
			const ownerNumber = ["6285259622026@s.whatsapp.net"] // ganti nomer lu
...
break
       ...
       client.sendMessage(from, 'Tuh kak no CO *POKS-BOT*, Kalau ada perlu atau sesuatu chat dia aja kak><',MessageType.text, { quoted: mek} )
           break
           case 'demote':
					if (!isGroup) return reply(mess.only.group)
					if (!isGroupAdmins) return reply(mess.only.admin)
				...
				case 'promote':
					...
					if (!isGroupAdmins) return reply(mess.only.admin)
					if (!isBotGroupAdmins) return reply(mess.only.Badmin)
					if (mek.message.extendedTextMessage === undefined || mek.message.extendedTextMessage === null) return reply('𝗧𝗮𝗴 𝘁𝗮𝗿𝗴𝗲𝘁 𝘆𝗮𝗻𝗴 𝗶𝗻𝗴𝗶𝗻 𝗱𝗶 promote!')
					mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid
					if (mentioned.length > 1) {
						teks = ''
						for (let _ of mentioned) {
